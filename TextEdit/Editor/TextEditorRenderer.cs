...
public class TextEditorRenderer
{
    ...
    const int CursorOnMs = 800;
    const int CursorPeriodMs = 1200;
    ...
    void RenderInner()
    {
        ...
        // Deduce _textStart from the line-number gutter width, or just the margin when line numbers are hidden
        float spaceWidth = _charWidthCache.Get(' ');
        _textStart = ShowLineNumbers
            ? ImGui.CalcTextSize(_lineNumberCache.Get(globalLineMax)).X + LeftMargin + spaceWidth
            : LeftMargin;
    ...
    )
    {
        ...
        if (_selection.Cursor.Line == lineNo)
        {
            ...
            if (focused)
            {
                if (!_selection.Cursor.Equals(_lastCursor))
                {
                    _startTime = DateTime.UtcNow;
                    _lastCursor = _selection.Cursor;
                }

                var elapsed = (DateTime.UtcNow - _startTime).TotalMilliseconds;
                var cursorVisible = !ImGui.GetIO().ConfigInputTextCursorBlink
                    || elapsed % CursorPeriodMs < CursorOnMs;
